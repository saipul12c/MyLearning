...
export default function InstructorEventDetailPage() {
  const params = useParams();
  const eventId = params.id as string;
  const { isInstructor, user, loading: authLoading } = useAuth();
  ...
  }, [isInstructor, eventId, user]);

  const fetchData = async () => {
    setLoading(true);
    // Only fetch event if it belongs to this instructor
  ...
  const handleExportCSV = () => {
    if (!registrants.length || !eventData) return;
    const headers = ['Nama', 'Role', 'Status Presensi', 'Status Pembayaran', 'Tanggal Daftar'];
    const rows = registrants.map(reg => [
      reg.profile?.full_name || 'Tanpa Nama',
      reg.profile?.role || '-',
      reg.status,
      reg.payment_status,
    ...
    a.href = url;
    a.download = `peserta_${eventData.slug || eventId}.csv`;
    a.click();
  ...
  if (!isInstructor || !eventData) {
    return (
      <div className="p-12 text-center card bg-red-500/5 border-red-500/10">
        <XCircle className="mx-auto text-red-500 mb-4" size={48} />
        <h2 className="text-xl font-bold text-white mb-2">Akses Ditolak</h2>
        <p className="text-slate-400">Event tidak ditemukan atau bukan milik Anda.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8 animate-fade-in pb-20">
      <Link href="/dashboard/instructor/events" className="inline-flex items-center gap-2 text-slate-500 hover:text-white transition-colors group">
        <ArrowLeft size={16} className="group-hover:-translate-x-1 transition-transform" />
        <span className="text-xs font-bold uppercase tracking-widest">Kembali ke Event Saya</span>
      </Link>
      ...
      <div className="card p-8 bg-[#0c0c14] border-white/5 space-y-4">
         ...
         <div className="flex gap-4 flex-wrap">
            ...
            <span className="px-3 py-1 rounded-full bg-white/5 text-[10px] font-black uppercase tracking-widest">{eventData?.category}</span>
         </div>
      </div>
         ...
         <div className="p-6 border-b border-white/5 flex justify-between items-center bg-white/[0.02]">
            ...
            <div className="flex items-center gap-4">
              <button 
                ...
                className="btn-secondary !h-8 !px-4 text-[10px] flex items-center gap-2"
              >
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="border-b border-white/5">
                  ...
                  <th className="px-6 py-4 text-[10px] font-black text-slate-500 uppercase tracking-widest">Status Presensi</th>
              ...
              <tbody className="divide-y divide-white/5">
                {registrants.map((reg) => (
                  <tr key={reg.id} className="hover:bg-white/[0.01]">
                    ...
                    <td className="px-6 py-4">
                       <span className={`px-2 py-1 rounded text-[10px] items-center gap-1 inline-flex font-black uppercase tracking-widest ${
         ...
         <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
            <div className="bg-[#0c0c14] border border-white/10 rounded-3xl w-full max-w-lg p-6 space-y-6">
                ...
                <div className="space-y-4">
                   ...
                   <div>
                      <label className="text-[10px] font-black uppercase tracking-widest text-slate-500 block mb-2">Catatan</label>
                      <textarea value={adminNotes} onChange={(e) => setAdminNotes(e.target.value)} rows={3} placeholder="Berikan catatan opsional..." className="input w-full resize-none"></textarea>
                   </div>
                </div>
