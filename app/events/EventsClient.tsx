...
import { getEvents, PlatformEvent, getEventSearchSuggestions } from "@/lib/events";
...
export default function EventsGalleryClient({ 
  initialEvents = [],
  initialListingPromo = null, 
  initialFeaturedPromo = null 
}: { 
  initialEvents?: PlatformEvent[],
  initialListingPromo?: Promotion | null, 
  initialFeaturedPromo?: Promotion | null 
}) {
  ...
  const filteredEvents = events.filter(e => {
    const matchesSearch = fuzzyMatch(e.title, searchQuery);
    ...
    <div className="min-h-screen bg-[#09090f] text-white selection:bg-purple-500/30">
      ...
      <section className="relative pt-32 pb-20 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative text-center">
          ...
          {/* Search Bar - Upgraded with Autocomplete */}
          <div className="max-w-2xl mx-auto relative group animate-fade-in-up [animation-delay:300ms]">
            <SearchAutocomplete 
              ...
              getSuggestions={getEventSearchSuggestions}
              onSuggestionClick={(s) => {
                setSearchQuery(s.title);
              }}
              historyKey="events_search_history"
            />
          </div>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-8 mb-20 relative z-20">
        <NativeAdCard 
          ...
          initialPromo={initialListingPromo}
        />
      </div>
          ...
          {loading ? (
            ...
            <div className={viewMode === 'grid' ? "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8" : "space-y-6"}>
              {filteredEvents.map((event, index) => (
                <EventCard key={event.id} event={event} mode={viewMode} index={index} query={searchQuery} />
              ))}
            </div>
          ) : (
...
function EventCard({ event, mode, index, query }: { event: PlatformEvent, mode: 'grid' | 'list', index: number, query: string }) {
    ...
    return (
      ...
      >
        ...
        <div className="flex-1 p-8 flex flex-col justify-center">
            ...
            <h3 className="text-2xl font-bold text-white mb-2 group-hover:text-purple-400 transition-colors line-clamp-1">
